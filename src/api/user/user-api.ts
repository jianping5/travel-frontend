...
export const register = async (req: RegisterReq): Promise<BaseRegisterResp> => {
  try {
    // 发送登录请求，并等待响应
    const response = await axios.post<BaseRegisterResp>(`${BASE_URL}/api/user/register`, req);
    // 非 200 报错
...
export const login = async (req: LoginReq): Promise<BaseLoginResp> => {
  try {
    // 发送登录请求，并等待响应
    const response = await axios.post<BaseLoginResp>(`${BASE_URL}/api/user/login`, req);
    // 非 200 报错
    if (response.data.code != "200") {
      throw new Error("状态码错误")
    }
...
export const getUserInfo = async (req: UserInfoReq): Promise<BaseUserInfoResp> => {
  try {
    // 发送登录请求，并等待响应
    const response = await axios.post<BaseUserInfoResp>(`${BASE_URL}/api/user/userinfo`, req);
    // 非 200 报错
    if (response.data.code != "200") {
      throw new Error("状态码错误")
    }
...
export const follow = async (req: FollowReq): Promise<BaseResp> => {
  try {
    // 发送登录请求，并等待响应
    const response = await axios.post<BaseResp>(`${BASE_URL}/api/user/follow`, req);
    // 非 200 报错
    if (response.data.code != "200") {
      throw new Error("状态码错误")
    }
...
export const getFollowList = async (req: FollowListReq): Promise<BaseFollowListView> => {
  try {
    // 发送登录请求，并等待响应
    const response = await axios.post<BaseFollowListView>(`${BASE_URL}/api/user/follow/list`, req);
    // 非 200 报错
    if (response.data.code != "200") {
      throw new Error("状态码错误")
    }
